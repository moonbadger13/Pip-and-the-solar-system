...
export function followMotion(distance,dt){dt=clamp(Number.isFinite(dt)?dt:0,0,.05);const travel=Math.min(Math.max(0,distance-2.8),dt*clamp((distance-2.8)*5,0,18));return{travel,pace:dt>0?travel/dt:0};}
export function smoothFollowPace(previous,pace,dt){return previous+(pace-previous)*(1-Math.exp(-Math.max(0,dt)*8));}
