...
for(let planet=0;planet<8;planet++){
 const g=new GameState();g.launch();assert(g.setCourse(planet));for(let i=0;i<1000&&g.target;i++)g.step(.05,{});assert.equal(g.mode,'foot');assert.equal(g.planet,planet);assert(g.visited.has(planet));
 ...
 g.launch();g.setCourse('station');for(let k=0;k<1000&&g.target;k++)g.step(.05,{});assert.equal(g.mode,'docked');const cargo=g.cargo;assert.equal(g.deposit(),cargo);assert.equal(g.cargo,0);assert.equal(g.deliveredColors[planet],cargo);assert(g.refuel());assert(g.undock());
}
...
g.land(4);for(let i=0;i<5000;i++)g.step(.05,{x:1,z:1,run:true});assert(g.supported||g.alt<0);assert(Number.isFinite(g.x));g.cargo=17;g.rescue();assert.equal(g.cargo,17);assert.equal(g.mode,'foot');g.launch();g.rocketFuel=0;const oldx=g.space.x;g.step(.05,{x:1});assert(g.space.x>oldx);assert(!g.setCourse(8));assert.throws(()=>g.land(-1));
