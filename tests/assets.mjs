...
for(const f of['game.js','models.js','planet-world.js','vendor/three.module.min.js']){const text=fs.readFileSync(new URL(f,root),'utf8');for(const m of text.matchAll(/from\s*['"](\.\/[^'"]+)['"]/g))assert(fs.existsSync(new URL(m[1],new URL(f,root))),m[1]);}
